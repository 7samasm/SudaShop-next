...
interface ITokenAndSession extends JWT, Session {
  accessToken: string;
...
export default NextAuth({
  callbacks: {
    // reform jwt
    async jwt(token: ITokenAndSession, user: IUser) {
    ...
    async session(session: ITokenAndSession, token: ITokenAndSession) {
    ...
    Providers.Credentials({
      async authorize(
        ...
        if (email && password) {
          ...
          if (session) {
            const { data } = await axiosBuilder().post<IUser>(
 ...
 */
async function refreshAccessToken(token: ITokenAndSession) {
  console.log(
    chalk.bgBlue.white(
